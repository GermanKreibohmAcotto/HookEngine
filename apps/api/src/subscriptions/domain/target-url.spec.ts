...
import { assertSafeTargetUrl, UnsafeTargetUrlError } from './target-url';
...
describe('assertSafeTargetUrl', () => {
  ...
  it('rejects loopback IPv4 literals', async () => {
    await expect(assertSafeTargetUrl('http://127.0.0.1/hook')).rejects.toThrow(
      UnsafeTargetUrlError,
    );
  });

  it('rejects the localhost hostname', async () => {
    await expect(assertSafeTargetUrl('http://localhost:3000/hook')).rejects.toThrow(
      UnsafeTargetUrlError,
    );
  });
  ...
  it.each(['10.0.0.5', '172.16.5.1', '192.168.1.1'])(
    ...
    async (address) => {
      await expect(assertSafeTargetUrl(`http://${address}/hook`)).rejects.toThrow(
        UnsafeTargetUrlError,
      );
    },
  );
  ...
  it('rejects the cloud metadata link-local address', async () => {
    await expect(assertSafeTargetUrl('http://169.254.169.254/latest/meta-data')).rejects.toThrow(
      UnsafeTargetUrlError,
    );
  });

  it('rejects IPv6 loopback', async () => {
    await expect(assertSafeTargetUrl('http://[::1]/hook')).rejects.toThrow(UnsafeTargetUrlError);
  });

  it('rejects an IPv4-mapped IPv6 loopback bypass attempt', async () => {
    await expect(assertSafeTargetUrl('http://[::ffff:127.0.0.1]/hook')).rejects.toThrow(
      UnsafeTargetUrlError,
    );
  });

  it('rejects non-http(s) protocols', async () => {
    await expect(assertSafeTargetUrl('ftp://example.com/hook')).rejects.toThrow(
      UnsafeTargetUrlError,
    );
  });

  it('rejects a malformed URL', async () => {
    await expect(assertSafeTargetUrl('not a url')).rejects.toThrow(UnsafeTargetUrlError);
  });
  ...
  it('rejects a hostname that resolves to a private address (DNS rebinding attempt)', async () => {
    ...
    await expect(assertSafeTargetUrl('https://sneaky.example.com/hook')).rejects.toThrow(
      UnsafeTargetUrlError,
    );
  });

  it('rejects a hostname that fails to resolve', async () => {
    await expect(assertSafeTargetUrl('https://does-not-exist.example.com/hook')).rejects.toThrow(
      UnsafeTargetUrlError,
    );
  });
});
